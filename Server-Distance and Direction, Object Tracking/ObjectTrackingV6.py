...
def assign_objects(old_frame, num_objects_old, new_frame, num_objects_new, t, v):
    ...
    if num_objects_new != 0:
        for i in range(0, num_objects_old):
            ...
            max_error = round(t * math.sqrt((old_frame['velocity (t)'][i, 0] ** 2) + (old_frame['velocity (t)'][i, 1] ** 2)) + 0.5 * max_acceleration_or_decceleration * (t ** 2), 3) + (2.2222 * int(old_frame['velocity (t)'][i, 0] == 0 and old_frame['velocity (t)'][i, 1] == 0))
        ...
        for i in range(num_objects_new, 50):
