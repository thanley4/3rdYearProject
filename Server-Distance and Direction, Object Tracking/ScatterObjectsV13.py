...
def update_frame(self, i):
    ...
    self.scatter_plot.set_offsets(objects_in_frame['position (t)'])
    self.scatter_plot_1.set_offsets(objects_in_frame['position (t-1)'])
    self.scatter_plot_2.set_offsets(objects_in_frame['position (t-2)'])
    self.scatter_plot_3.set_offsets(objects_in_frame['position (t-3)'])
    self.scatter_plot_4.set_offsets(objects_in_frame['position (t-4)'])
    self.scatter_plot_5.set_offsets(objects_in_frame['position (t-5)'])
    self.scatter_plot_6.set_offsets(objects_in_frame['position (t-6)'])

    # Update Scatter Plot Sizes
    self.scatter_plot.set_sizes(objects_in_frame['size'])

    # Update Scatter Plot Colours
    self.scatter_plot.set_facecolors(objects_in_frame['color'])
...
class MyPlotClass:
    ...
    def __init__(self):
        ...
        self.scatter_plot = ax.scatter(objects_in_frame['position (t)'][:, 0],
                                       objects_in_frame['position (t)'][:, 1],
                                       s=objects_in_frame['size'],
                                       edgecolors=[0, 0, 0],
                                       facecolors=objects_in_frame['color'])

        self.scatter_plot_1 = ax.scatter(objects_in_frame['position (t-1)'][:, 0],
                                         objects_in_frame['position (t-1)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        self.scatter_plot_2 = ax.scatter(objects_in_frame['position (t-2)'][:, 0],
                                         objects_in_frame['position (t-2)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        self.scatter_plot_3 = ax.scatter(objects_in_frame['position (t-3)'][:, 0],
                                         objects_in_frame['position (t-3)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        self.scatter_plot_4 = ax.scatter(objects_in_frame['position (t-4)'][:, 0],
                                         objects_in_frame['position (t-4)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        self.scatter_plot_5 = ax.scatter(objects_in_frame['position (t-5)'][:, 0],
                                         objects_in_frame['position (t-5)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        self.scatter_plot_6 = ax.scatter(objects_in_frame['position (t-6)'][:, 0],
                                         objects_in_frame['position (t-6)'][:, 1],
                                         s=10, edgecolors=[0, 1, 1], facecolors="None")

        # Decorative Graphics

        # Field of View
        ax.plot([417, 960, 1503], [0, 900, 0], color=(0.8, 0.8, 0.8, 0.8), linestyle=":")
        ...
        # Shading
        ax.fill((0, 0, 1920, 1920, 1503, 960, 417), (0, 1080, 1080, 0, 0, 900, 0), color=(0.8, 0.8, 0.8, 0.8))

        # Car Fill
        ax.fill((944, 976, 976, 944), (884, 884, 964, 964), color=(0, 0, 1, 1), zorder=10)

        # Car Outline
        ax.plot((944, 976, 976, 944, 944), (884, 884, 964, 964, 884), color=(0, 0, 0, 1), zorder=10)

        # Warning Area
        ax.fill((960, 856.69, 875.48, 891.60, 908.24, 925.27, 942.57, 960.00, 977.43, 994.73, 1011.76, 1028.40, 1044.52,
                 1063.31),
                (900, 728.75, 718.74, 712.06, 706.81, 703.04, 700.76, 700.00, 700.76, 703.04, 706.81, 712.06, 718.74,
                 728.75),
                color=(1, 0, 0, 0.5))
...
# class DataFetchClass(multiprocessing.Process):
#
#     def __init__(self, ):
...
class WarningClass(multiprocessing.Process):
    global num_objects_in_frame, objects_in_frame
    ...
    def run(self):
        ...
        while True:
            for i in range(0, num_objects_in_frame):
                if objects_in_frame['position (t+1)'][i, 0] in range(860, 1060) and objects_in_frame['position (t+1)'][
                    i, 1] in range(700, 900):
                    beeper = BeepingClass()
                    beeper.start()
                    beeper.join()
...
if __name__ == '__main__':
    ...
    plotter = MyPlotClass()
    print("2")
    # fetcher = DataFetchClass()
    # warner = WarningClass()
    plt.show()
    print("3")
    process = Process(target=run_fetch())
    process.start()
    print("4")
    # fetcher.start()
    # warner.start()
