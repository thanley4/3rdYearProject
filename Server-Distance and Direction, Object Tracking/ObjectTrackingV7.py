...
def find_match(portion, index):
    ...
    global num_objects_new, new_frame, t
    ...
    return match['error'], match['old index'], match['new index']


def assign_objects(old_frame, num_objects_old, new_frame_passed, num_objects_new_passed, t_passed, v):
    print("Received Object with Num New Objects: ", num_objects_new_passed)
    old_frame['match'] = False
    global num_total_matches
    global num_total_objects
    global num_objects_new
    global new_frame
    global t

    num_objects_new = num_objects_new_passed
    new_frame = new_frame_passed
    t = t_passed
    ...
    if num_objects_new != 0:
        if num_objects_old != 0:
            with get_pool(num_objects_old) as pool:
                matches['error'], matches['old index'], matches['new index'] = pool.starmap(find_match, zip(old_frame[0:num_objects_old], np.arange(num_objects_old)))
                # pool.close()
        ...
        # Sorted puts max error at bottom of 100 long vector
            ...
            old_frame['velocity (t)'][:, 1] = ((old_frame['position (t)'][:, 1] - old_frame['position (t-1)'][:,
                                                                                  1]) * 1.5) - v
        j = 0
