...
Usage:
    $ python path/to/detect.py --source path/to/img.jpg --weights yolov5s.pt --img 320
"""
...
from utils.additional_functions import Object, ObjectList, object_distance, object_angle, object_width, send_objects
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c < 20:
                            ...
                            width_in_pixels = int(xyxy[2] - xyxy[0])
                            x_location = float(((xyxy[2] - xyxy[0]) / 2) + xyxy[0])
                            object_detected = Object(c, height_in_pixels, width_in_pixels, x_location)
                            objects_in_frame.append(object_detected)
                            # send_objects(object_detected)
                            label = None if hide_labels else (names[
                                                                  c] if hide_conf else f'{object_detected.width} {object_detected.distance} {object_detected.angle} {names[c]} {conf:.2f}')
                        else:
            ...
            send_objects(ObjectList(objects_in_frame))
