...
def assign_objects(old_frame, num_objects_old, new_frame, num_objects_new, t, v):
    ...
    #     old_frame['position (t)'][i] = new_frame['position (t)'][i]
    #     old_frame['id'][i] = new_frame['id'][i]
    #     old_frame['size'][i] = new_frame['size'][i]
    #
    # print("RETURNING")
    ...
    if num_objects_new != 0:
        for i in range(0, num_objects_old):
            print("I: ", i)
            matches.append(find_match(old_frame[i], i))
