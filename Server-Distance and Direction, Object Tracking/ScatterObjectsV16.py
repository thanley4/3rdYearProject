...
from utils.additional_functionsV4 import object_colours, deserialize
from ObjectTrackingV11 import assign_objects
...
lane = np.zeros(1, dtype=[('left', float, (2,)),
                          ('right', float, (2,)),
                          ('length', float)])
...
class MyPlotClass():
    ...
    def __init__(self, dataClass):
        ...
        self.scatter_plot_7 = ax.scatter(self._dataClass.lane['left'][:, 0], self._dataClass.lane['left'][:, 1], s=10, edgecolor=[0, 1, 0],
                                         facecolors=[0, 1, 0])

        self.scatter_plot_8 = ax.scatter(self._dataClass.lane['right'][:, 0], self._dataClass.lane['right'][:, 1], s=10, edgecolor=[0, 1, 0],
                                         facecolors=[0, 1, 0])
        ...
        warning_area = ax.fill((960, 856.69, 875.48, 891.60, 908.24, 925.27, 942.57, 960.00, 977.43, 994.73, 1011.76,
                                1028.40, 1044.52, 1063.31),
                               (900, 728.75, 718.74, 712.06, 706.81, 703.04, 700.76, 700.00, 700.76, 703.04, 706.81,
                                712.06, 718.74, 728.75),
                               color=(1, 0, 0, 0.5))
    ...
    def update(self, i):
        ...
        self.scatter_plot_7.set_offsets(self._dataClass.lane['left'])
        self.scatter_plot_8.set_offsets(self._dataClass.lane['right'])
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        packet_id, drowsiness, lane['left'], lane['right'], ids, distances, angles, widths = deserialize(
            self.data.decode())
